...
export class AuthService {
  ...
  async register(props) {
    try {
      ...
      const newUserId = await this.userService.createUser(
        new CreateUserDTO({
          ...props,
          password: await props.hashPassword(props.password),
        })
      );
  ...
  async login(props) {
    ...
    if (!isExist) throw { status: 404, message: "존재하지 않은 이메일입니다" };

    const isCorrect = await props.comparePassword(isExist.password);

    if (!isCorrect)
      throw { status: 400, message: "비밀번호를 잘못 입력하셨습니다" };
  ...
  async refresh(accessToken, refreshToken) {
    ...
    const user = await this.userService.findById(accessTokenPayload.id);
